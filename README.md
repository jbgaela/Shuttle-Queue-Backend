...
Public ranking traffic is forwarded by the frontend Netlify Edge Function. Set the same random, server-only `PUBLIC_RANKING_EDGE_SECRET` (at least 32 characters) on Netlify and Render before deploying this blueprint. The secret remains a non-synchronized Render setting. The blueprint declares `PUBLIC_RANKING_ACCESS_ENABLED=true` and `PUBLIC_RANKING_LOCATION_REQUIRED=true` so later Blueprint synchronization cannot silently disable an activated service. Production public-ranking requests without a valid signature are rejected. Development requests use the socket IP with no approximate location.
...
Deploy in a coordinated window: create one secret (PowerShell: `$secret = [Convert]::ToHexString([Security.Cryptography.RandomNumberGenerator]::GetBytes(32))`), enter the same value in Render and Netlify without printing it, and confirm Netlify's variable has Functions scope in the target deploy context. Set Netlify `BACKEND_API_BASE_URL` to the Render HTTPS API URL ending in `/api/v2`. Deploy Render, then redeploy Netlify so its edge function captures the corrected variables. An invalid-token request through Netlify must reach Render and return `404`; `503 EDGE_CONFIGURATION_ERROR` indicates an invalid edge setting. Verify a valid publication before completing activation. The public frontend and API both require a saved location. Private tracking requires a signed-in owner or Super Admin. Approved browser-location visits expire after 12 hours; raw visit keys are never stored. Location reports and user agents are unverified client reports.
