...
export function historyMatchView(match: any) {
  ...
  const matchmakingLabel = match.matchmakingMode === "BALANCED" ? `Handicap +${[1, 2, 3].includes(strengthGap) ? strengthGap : 1}` : match.matchmakingMode === "GUIDED" ? "Guided" : match.matchmakingMode === "UNDEFEATED_CHALLENGE" ? challengeLabel : match.matchmakingMode === "SAME_SKILL" ? "Same skill" : match.matchmakingMode === "MIXED_DOUBLES" ? "Mixed doubles" : match.matchmakingMode === "SAME_GENDER" ? "Same gender" : match.matchmakingMode === "OPEN" ? "Open" : match.source === "MANUAL_ADJUSTED" ? "Manual Adjusted" : "Manual";
