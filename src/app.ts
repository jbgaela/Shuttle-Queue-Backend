...
import { isProhibitedGeneratedGenderMatch, MATCHMAKING_ALGORITHM, suggestMatch, validateBalancedLineup, type MatchHistory, type MatchPlayer, type MatchmakingOptions } from "./lib/matchmaking.js";
...
async function createMatch(request: Request, body: { teamA: string[]; teamB: string[]; courtId?: string; suggestionToken?: string; suggestionAdjusted?: boolean; suggestionPayload?: Record<string, unknown> }) {
  ...
  const matchId = await withTransactionRetry(async (tx) => {
    ...
    const suggestionExplanation = body.suggestionPayload ? { algorithmVersion: MATCHMAKING_ALGORITHM, strengthGap: body.suggestionPayload.strengthGap ?? null, mode: body.suggestionPayload.mode ?? null, adjusted: Boolean(body.suggestionAdjusted), generatedGenderRule: "NO_TWO_FEMALE_VS_TWO_MALE", rest: { minimumRestMinutes: settings.minimumRestMinutes } } : null;
