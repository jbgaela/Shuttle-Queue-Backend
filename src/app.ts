...
import { isProhibitedGeneratedGenderMatch, loneFemalePolicy, lowSkillLoneFemaleAdvisory, MATCHMAKING_ALGORITHM, suggestMatch, undefeatedChallengePlayers, validateBalancedLineup, validateMixedDoublesLineup, validateSynergyLineup, type MatchHistory, type MatchPlayer, type MatchmakingOptions } from "./lib/matchmaking.js";
...
function matchView(match: any) {
  ...
  const generatedOrigin = match.source === MatchSource.AUTOMATIC || explanation?.generatedOrigin === "SUGGESTION";
  ...
  const matchupAdvisory = generatedOrigin ? lowSkillLoneFemaleAdvisory(
...
function signSuggestion(payload: Record<string, unknown>) { const body = Buffer.from(JSON.stringify({ ...payload, algorithmVersion: MATCHMAKING_ALGORITHM })).toString("base64url"); return `${body}.${hmac(body)}`; }
...
async function validateSuggestionRequest(request: Request, tokenValue: string, teamA: string[], teamB: string[], adjusted: boolean) {
  ...
  if (token.algorithmVersion !== MATCHMAKING_ALGORITHM || token.queueMasterId !== authUser(request).id || token.revision !== workspace.matchmakingRevision || Number(token.expiresAt) < Date.now() || !validMode || !validStrength || typeof token.key !== "string" || !Array.isArray(token.teamA) || !Array.isArray(token.teamB) || (!adjusted && (JSON.stringify(token.teamA) !== JSON.stringify(teamA) || JSON.stringify(token.teamB) !== JSON.stringify(teamB)))) throw conflict("SUGGESTION_STALE", "Generate a new suggestion.");
  return token;
}
...
async function historyMaps(queueMasterId: string): Promise<MatchHistory> {
  ...
  const synergyTeams = await loadSynergyTeams(queueMasterId);
  ...
  return { partners, opponents, quartets, recentPartners, recentOpponents, recentQuartets, synergyTeams } as MatchHistory & { synergyTeams: SynergyTeamRecord[] };
}
...
async function createMatch(request: Request, body: { teamA: string[]; teamB: string[]; courtId?: string; suggestionToken?: string; suggestionAdjusted?: boolean; playerPreferenceConfirmed?: boolean; suggestionPayload?: Record<string, unknown> }) {
  ...
  const matchId = await withTransactionRetry(async (tx) => {
    ...
    const synergyError = validateSynergyLineup(body.teamA.map(toMatchPlayer), body.teamB.map(toMatchPlayer), synergyTeams as any);
    ...
    if (body.suggestionPayload) {
      ...
      if (suggestionSynergyError) throw conflict("SYNERGY_TEAM_LINEUP", suggestionSynergyError);
...
api.post("/matches/:id/start", requireAuth, requireMutationOrigin, route(async (request, response) => {
  ...
  const startedId = await withTransactionRetry(async (tx) => {
    ...
    const generatedOrigin = match.source === MatchSource.AUTOMATIC || startExplanation?.generatedOrigin === "SUGGESTION";
    const startAdvisory = generatedOrigin ? lowSkillLoneFemaleAdvisory(startTeamA, startTeamB) : null;
    ...
    if (startSynergyError) throw conflict("SYNERGY_TEAM_LINEUP", startSynergyError);
    if (match.matchmakingMode === MatchmakingMode.BALANCED || match.matchmakingMode === MatchmakingMode.MIXED_DOUBLES) {
      ...
      if (lineupSynergyError) throw conflict("SYNERGY_TEAM_LINEUP", lineupSynergyError);
...
api.patch("/matches/:id", requireAuth, requireMutationOrigin, route(async (request, response) => {
  ...
  const updatedId = await withTransactionRetry(async (tx) => {
    ...
    if (synergyError) throw conflict("SYNERGY_TEAM_LINEUP", synergyError);
    if (match.source === MatchSource.AUTOMATIC && isProhibitedGeneratedGenderMatch(teamAPlayers, teamBPlayers)) throw conflict("GENERATED_GENDER_RULE", "Generated matchups cannot place two female players against two male players.");
    ...
    const generatedOrigin = match.source === MatchSource.AUTOMATIC || previousExplanation?.generatedOrigin === "SUGGESTION";
    const updatedAdvisory = generatedOrigin ? lowSkillLoneFemaleAdvisory(teamAPlayers, teamBPlayers) : null;
    ...
    await tx.match.update({ where: { id: match.id, version: expected }, data: { ...courtData, source: MatchSource.MANUAL_ADJUSTED, matchmakingMode: preservedBalanced ? MatchmakingMode.BALANCED : null, algorithmVersion: preservedBalanced ? (match.algorithmVersion ?? MATCHMAKING_ALGORITHM) : null, suggestionKey: null, suggestionExplanation: generatedOrigin ? { ...(previousExplanation ?? {}), generatedOrigin: "SUGGESTION", algorithmVersion: match.algorithmVersion ?? MATCHMAKING_ALGORITHM, adjusted: true, matchupAdvisory: updatedAdvisory } : preservedBalanced ? { ...(previousExplanation ?? {}), algorithmVersion: match.algorithmVersion ?? MATCHMAKING_ALGORITHM, adjusted: true } : null, version: { increment: 1 } } });
