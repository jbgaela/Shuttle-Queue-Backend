...
import { buildGuidedExplanation, evaluateGuidedAvailability, isProhibitedGeneratedGenderMatch, isProhibitedGeneratedNewbieMatch, loneFemalePolicy, lowSkillLoneFemaleAdvisory, MATCHMAKING_ALGORITHM, suggestMatch, undefeatedChallengePlayers, UNDEFEATED_CHALLENGE_MINIMUM_MATCHES, UNDEFEATED_CHALLENGE_RANK_LIMIT, validateBalancedLineup, validateGuidedLineup, validateMatchmakingConstraints, validateMixedDoublesLineup, validateSynergyLineup, type MatchHistory, type MatchPlayer, type MatchmakingOptions } from "./lib/matchmaking.js";
...
const suggestionRequestSchema = z.discriminatedUnion("mode", [
  z.strictObject({ mode: z.literal(MatchmakingMode.GUIDED), excludeKeys: z.array(z.string()).max(50).default([]) }),
  z.strictObject({ mode: z.literal(MatchmakingMode.BALANCED), strengthGap: z.union([z.literal(1), z.literal(2), z.literal(3)]).optional(), excludeKeys: z.array(z.string()).max(50).default([]) }),
  z.strictObject({ mode: z.enum([MatchmakingMode.OPEN, MatchmakingMode.SAME_SKILL, MatchmakingMode.SAME_GENDER, MatchmakingMode.MIXED_DOUBLES, MatchmakingMode.UNDEFEATED_CHALLENGE]), excludeKeys: z.array(z.string()).max(50).default([]) }),
]);
...
const cloudSnapshotSchema = z.strictObject({
  ...
  synergyTeams: z.array(z.strictObject({ id: z.string(), queuePlayerIds: z.tuple([z.string(), z.string()]), createdAt: z.string().datetime(), version: z.number().int() })).default([]),
  ...
  feeConfig: z.strictObject({ id: z.string(), mode: z.nativeEnum(FeeMode), currencyCode: z.string(), fixedAmountPerPlayerMinor: z.number().int().nullable(), expectedQueueCostMinor: z.number().int().nullable(), noShowPenaltyMinor: noShowPenaltySnapshotSchema, participationRule: z.string(), frozenAt: z.string().datetime().nullable(), version: z.number().int() }).nullable(),
...
const parse = <T>(schema: z.ZodType<T>, value: unknown) => {
  ...
  if (parsed && typeof parsed === "object" && "snapshot" in parsed && (parsed as Record<string, unknown>).schemaVersion === 2) {
    ...
    for (const team of snapshot.synergyTeams ?? []) {
      if (team.queuePlayerIds[0] === team.queuePlayerIds[1] || team.queuePlayerIds.some((id) => !queuePlayerIds.has(id)) || team.queuePlayerIds.some((id) => synergyMembers.has(id))) throw new AppError(422, "VALIDATION_ERROR", "The snapshot contains an invalid Synergy Team reference.");
      team.queuePlayerIds.forEach((id) => synergyMembers.add(id));
    }
...
const responseData = (response: Response, data: unknown, status = 200, meta?: unknown) => {
  const value = data && typeof data === "object" ? data as Record<string, unknown> : null;
  const suggestion = value?.suggestion && typeof value.suggestion === "object" ? value.suggestion as Record<string, unknown> : null;
  const teamA = suggestion?.teamA;
  const teamB = suggestion?.teamB;
  const advisory = suggestion && Array.isArray(teamA) && Array.isArray(teamB)
    ? lowSkillLoneFemaleAdvisory(teamA as Array<{ id: string; displayName: string; gender: string; skillLevel: string }>, teamB as Array<{ id: string; displayName: string; gender: string; skillLevel: string }>)
    : null;
  const noMatch = value?.noMatch && typeof value.noMatch === "object" ? value.noMatch as Record<string, unknown> : null;
  const challengeNoMatch = noMatch?.message === "No valid Undefeated Challenge lineup is available under the challenge rules."
    ? { ...noMatch, message: "No legal +1, +2, or equal-strength Undefeated Challenge lineup is available under the current queue constraints." }
    : noMatch;
  const hasSuggestionField = Boolean(value && Object.prototype.hasOwnProperty.call(value, "suggestion"));
  const enriched = hasSuggestionField ? { ...value, matchupAdvisory: advisory, ...(challengeNoMatch ? { noMatch: challengeNoMatch } : {}), ...(suggestion ? { suggestion: { ...suggestion, matchupAdvisory: advisory } } : {}) } : data;
  return response.status(status).json({ data: enriched, ...(meta === undefined ? {} : { meta }), requestId: response.locals.requestId });
};
...
api.put("/sync/snapshot", requireAuth, requireMutationOrigin, (request, response, next) => {
  ...
  // The transaction callback builds the merged snapshot before its writes are
  // committed. Reload the persisted Synergy Teams after commit so every sync
  // response contains the same team collection as a normal snapshot download.
  const responseSnapshot = await snapshotWithSynergyTeams(result.snapshot ?? await snapshotWithMatchSnapshots(await buildSnapshot(queueMasterId)));
  responseData(response, { cloudRevision: result.state.cloudRevision, lastSyncedAt: result.state.lastSyncedAt, schemaVersion: 3, alreadyApplied: result.alreadyApplied, snapshot: responseSnapshot, metadata: result.metadata });
  })(request, response, next);
});
api.get("/sync/snapshot", requireAuth, route(async (request, response) => { const snapshot = await snapshotWithSynergyTeams(await snapshotWithMatchSnapshots(await buildSnapshot(authUser(request).id))); const state = await db.accountSyncState.upsert({ where: { queueMasterId: authUser(request).id }, create: { queueMasterId: authUser(request).id, schemaVersion: 2 }, update: {} }); const upgraded = state.schemaVersion >= 3; const responseSnapshot = upgraded ? { ...snapshot, schemaVersion: 3 as const } : snapshot; responseData(response, { snapshot: responseSnapshot, checksum: snapshotChecksum(responseSnapshot), cloudRevision: state.cloudRevision, schemaVersion: upgraded ? 3 : 2, ...(upgraded ? { metadata: state.mergeMetadata } : {}) }); }));
api.get("/sync/snapshot-legacy", requireAuth, route(async (request, response) => {
  const queueMasterId = authUser(request).id;
...
async function ensureWorkspace(queueMasterId: string, database = db) {
  ...
  await database.queueFeeConfig.upsert({ where: { queueMasterId }, create: { queueMasterId, mode: settings.defaultFeeMode, currencyCode: settings.currencyCode, fixedAmountPerPlayerMinor: settings.defaultFixedFeeMinor, expectedQueueCostMinor: 0, noShowPenaltyMinor: settings.noShowPenaltyMinor ?? 0 }, update: {} });
...
function settingsView(settings: any) { return settings ? { id: settings.id, pointsToWin: settings.pointsToWin, winBy: settings.winBy, scoreCap: settings.scoreCap, bestOf: settings.bestOf, minimumRestMinutes: settings.minimumRestMinutes, lateArrivalGraceMinutes: settings.lateArrivalGraceMinutes ?? DEFAULT_LATE_ARRIVAL_GRACE_MINUTES, defaultFeeMode: settings.defaultFeeMode, defaultFixedFeeMinor: settings.defaultFixedFeeMinor, noShowPenaltyMinor: settings.noShowPenaltyMinor ?? 0, currencyCode: settings.currencyCode, timeZone: settings.timeZone, defaultLateArrivalCutoffTime: settings.defaultLateArrivalCutoffTime, version: settings.version } : null; }
function playerView(player: any) { return { id: player.id, displayName: player.displayName, gender: player.gender, skillLevel: player.skillLevel, skillWeight: skillWeight(player.skillLevel as SkillLevel), status: player.status, version: player.version }; }
...
type SynergyTeamRecord = { id: string; queueMasterId?: string; queuePlayerIds: string[]; createdAt: Date; updatedAt?: Date; version: number };
function validSynergyTeam(team: any): team is SynergyTeamRecord { return Boolean(team?.id && Array.isArray(team.queuePlayerIds) && team.queuePlayerIds.length === 2 && typeof team.queuePlayerIds[0] === "string" && typeof team.queuePlayerIds[1] === "string" && team.queuePlayerIds[0] !== team.queuePlayerIds[1]); }
function synergyTeamMap(teams: SynergyTeamRecord[]) {
  const map = new Map<string, SynergyTeamRecord>();
  for (const team of teams) if (validSynergyTeam(team) && !map.has(team.queuePlayerIds[0]) && !map.has(team.queuePlayerIds[1])) { map.set(team.queuePlayerIds[0], team); map.set(team.queuePlayerIds[1], team); }
  return map;
}
function effectiveSkillLevel(weight: number, fallback: SkillLevel): SkillLevel { return (Object.entries({ NEWBIE: 1, BEGINNER: 2, UPPER_BEGINNER: 3, INTERMEDIATE: 4, UPPER_INTERMEDIATE: 5, ADVANCED: 6 }).find(([, value]) => value === weight)?.[0] ?? fallback) as SkillLevel; }
function matchmakingPlayerFromQueuePlayer(player: any, byId: Map<string, any>, synergyTeams: SynergyTeamRecord[]): MatchPlayer {
  const rawWeight = skillWeight(player.skillLevelSnapshot as SkillLevel);
  const synergyTeam = synergyTeams.find((team) => team.queuePlayerIds.includes(player.id));
  const partnerId = synergyTeam?.queuePlayerIds.find((id) => id !== player.id);
...
async function loadSynergyTeams(queueMasterId: string, database = db): Promise<SynergyTeamRecord[]> {
  ...
  return rows.filter(validSynergyTeam);
}
function synergyTeamView(team: SynergyTeamRecord, queuePlayersById: Map<string, any>) {
  const [firstId, secondId] = team.queuePlayerIds;
  const first = queuePlayersById.get(firstId);
  const second = queuePlayersById.get(secondId);
  const firstWeight = first ? skillWeight(first.skillLevelSnapshot as SkillLevel) : 0;
  const secondWeight = second ? skillWeight(second.skillLevelSnapshot as SkillLevel) : 0;
  const effectiveWeight = Math.max(firstWeight, secondWeight);
  return { id: team.id, queuePlayerIds: [firstId, secondId] as [string, string], effectiveSkillWeight: effectiveWeight, effectiveSkillLevel: effectiveSkillLevel(effectiveWeight, (first?.skillLevelSnapshot ?? second?.skillLevelSnapshot ?? SkillLevel.BEGINNER) as SkillLevel), version: team.version, createdAt: team.createdAt, updatedAt: team.updatedAt ?? team.createdAt };
}
async function snapshotWithSynergyTeams(snapshot: CloudSnapshotV2): Promise<CloudSnapshotV2> {
  const teams = await loadSynergyTeams(snapshot.queueMasterId);
...
function undefeatedChallengeStatus(players: any[], minimumRestMinutes: number, teamsByPlayer = new Map<string, SynergyTeamRecord>(), reference = Date.now()) {
  ...
  return {
    ...
    players: ranked.map(({ player, rank }) => {
      const source = players.find((item) => item.id === player.id);
      const eligibleAt = restEligibleAt(source?.lastMatchEndedAt, minimumRestMinutes, now);
      return { queuePlayerId: player.id, displayName: player.displayName, rank, matchesPlayed: player.gamesPlayed, wins: player.wins ?? 0, losses: player.losses ?? 0, status: source?.status ?? player.status, ready: source?.status === QueuePlayerStatus.WAITING && eligibleAt.getTime() <= now, restEligibleAt: eligibleAt };
    }),
  };
}
...
function undefeatedChallengeNotificationEvents(before: MatchPlayer[], after: MatchPlayer[]) {
  ...
  return undefeatedChallengePlayers(after).filter(({ player }) => !previous.has(player.id)).map(({ player, rank }) => ({ type: "UNDEFEATED_CHALLENGE_ELIGIBLE" as const, queuePlayerId: player.id, displayName: player.displayName, rank, matchesPlayed: player.gamesPlayed, wins: player.wins ?? 0, losses: player.losses ?? 0 }));
}
function challengeNoMatchDetails(players: MatchPlayer[], minimumRestMinutes: number, excludedKeys: string[] = []) {
  const ranked = undefeatedChallengePlayers(players);
  const now = Date.now();
  const waiting = players.filter((player) => player.status === QueuePlayerStatus.WAITING);
  const ready = waiting.filter((player) => restEligibleAt(player.lastMatchEndedAt, minimumRestMinutes, now).getTime() <= now);
  const readyQualifierCount = ranked.filter(({ player }) => player.status === QueuePlayerStatus.WAITING && restEligibleAt(player.lastMatchEndedAt, minimumRestMinutes, now).getTime() <= now).length;
  const reason = ranked.length === 0
    ? "NO_QUALIFIER" as const
    : readyQualifierCount === 0
      ? "QUALIFIERS_UNAVAILABLE" as const
      : ready.length < 4
        ? "INSUFFICIENT_READY_PLAYERS" as const
        : excludedKeys.length > 0
          ? "ALTERNATIVES_EXHAUSTED" as const
          : "INCOMPATIBLE_LINEUP_CONSTRAINTS" as const;
  return { qualifierCount: ranked.length, readyQualifierCount, readyPlayerCount: ready.length, reason };
}
function validateUndefeatedChallengeStart(match: any, currentPlayers: any[]) {
  if (match.source !== MatchSource.AUTOMATIC) return;
  ...
  const qualifiedIds = new Set(undefeatedChallengePlayers(currentPlayers.map((player) => challengeInput(player))).map(({ player }) => player.id));
  if (!selectedIds.every((id) => qualifiedIds.has(id))) throw conflict("UNDEFEATED_CHALLENGE_CONSTRAINT", "One or more undefeated players no longer qualify for this challenge. Continue as Manual Adjusted or generate a new challenge.", { canConvertToManual: true, originalMode: MatchmakingMode.UNDEFEATED_CHALLENGE });
}
...
function matchView(match: any) {
  const participants = (match.participants ?? []).map((participant: any) => ({ id: participant.id, queuePlayerId: participant.queuePlayerId, displayName: participant.queuePlayer?.displayNameSnapshot, gender: participant.queuePlayer?.genderSnapshot, skillLevel: participant.queuePlayer?.skillLevelSnapshot, playerStatus: participant.queuePlayer?.status, lastMatchEndedAt: participant.queuePlayer?.lastMatchEndedAt, team: participant.team, teamSlot: participant.teamSlot }));
  const toAdvisoryPlayer = (participant: any) => ({ id: participant.queuePlayerId, displayName: participant.displayName ?? "Player", gender: participant.gender, skillLevel: participant.skillLevel });
  const matchupAdvisory = lowSkillLoneFemaleAdvisory(
    participants.filter((participant: any) => participant.team === TeamSide.A).map(toAdvisoryPlayer),
    participants.filter((participant: any) => participant.team === TeamSide.B).map(toAdvisoryPlayer),
  );
  return { id: match.id, status: match.status, source: match.source, courtId: match.courtId, matchmakingMode: match.matchmakingMode, algorithmVersion: match.algorithmVersion, suggestionKey: match.suggestionKey, suggestionExplanation: match.suggestionExplanation, matchupAdvisory, queuedAt: match.queuedAt, startedAt: match.startedAt, completedAt: match.completedAt, cancelledAt: match.cancelledAt, cancellationReason: match.cancellationReason, winnerTeam: match.winnerTeam, currentRevisionId: match.currentRevisionId, scoring: scoreSettings(match), version: match.version, participants };
}
function workspaceView(workspace: any, settings: any, counts: any, feeConfig?: any) { return { startedAt: workspace.startedAt, endedAt: workspace.endedAt, status: workspace.endedAt ? "ENDED" : "ACTIVE", lateArrivalCutoffAt: workspace.lateArrivalCutoffAt, matchmakingAlgorithm: workspace.matchmakingAlgorithm, matchmakingRevision: workspace.matchmakingRevision, version: workspace.version, playerCount: counts?.queuePlayers ?? 0, courtCount: counts?.courts ?? 0, scoring: scoreSettings(settings), feeConfig: feeConfig ? { id: feeConfig.id, mode: feeConfig.mode, currencyCode: feeConfig.currencyCode, fixedAmountPerPlayerMinor: feeConfig.fixedAmountPerPlayerMinor, expectedQueueCostMinor: feeConfig.expectedQueueCostMinor, noShowPenaltyMinor: feeConfig.noShowPenaltyMinor ?? 0, participationRule: feeConfig.participationRule, frozenAt: feeConfig.frozenAt, version: feeConfig.version } : null }; }
...
function signSuggestion(payload: Record<string, unknown>) { return signSuggestionToken(payload, config.suggestionSigningSecret, MATCHMAKING_ALGORITHM); }
function verifySuggestion(value: string) { try { return parseAndVerifySuggestionToken(value, config.suggestionSigningSecret); } catch { throw conflict("SUGGESTION_STALE", "Generate a new suggestion."); } }
function validateGuidedMutation(teamA: MatchPlayer[], teamB: MatchPlayer[], synergyError?: string | null) {
  const compositionError = validateGuidedLineup(teamA, teamB);
  if (compositionError) throw conflict("GUIDED_COMPOSITION", compositionError, { canConvertToManual: true, originalMode: MatchmakingMode.GUIDED });
  if (synergyError) throw conflict("SYNERGY_TEAM_LINEUP", synergyError, { canConvertToManual: true, originalMode: MatchmakingMode.GUIDED });
  if (isProhibitedGeneratedGenderMatch(teamA, teamB)) throw conflict("GENERATED_GENDER_RULE", "Generated matchups cannot place two female players against two male players.", { canConvertToManual: true, originalMode: MatchmakingMode.GUIDED });
}
function validateModeGuarantee(mode: MatchmakingMode, teamA: MatchPlayer[], teamB: MatchPlayer[], strengthGap = 1) {
  const result = validateMatchmakingConstraints(mode, teamA, teamB, strengthGap);
  return result.valid ? null : result.message;
}
async function validateSuggestionRequest(request: Request, tokenValue: string, teamA: string[], teamB: string[], adjusted: boolean, overrideToManual = false) {
  ...
  const expiresAt = typeof token.expiresAt === "number" || typeof token.expiresAt === "string" ? Number(token.expiresAt) : Number.NaN;
  const validExpiry = Number.isFinite(expiresAt) && expiresAt >= Date.now();
  const validTeams = Array.isArray(token.teamA) && Array.isArray(token.teamB) && token.teamA.length === 2 && token.teamB.length === 2 && token.teamA.every((id) => typeof id === "string") && token.teamB.every((id) => typeof id === "string") && new Set([...token.teamA, ...token.teamB]).size === 4;
  const validKey = validTeams && typeof token.key === "string" && token.key === `${[...token.teamA as string[]].sort().join(",")}|${[...token.teamB as string[]].sort().join(",")}`;
  const lineupChanged = JSON.stringify(token.teamA) !== JSON.stringify(teamA) || JSON.stringify(token.teamB) !== JSON.stringify(teamB);
  if (token.algorithmVersion !== MATCHMAKING_ALGORITHM || token.queueMasterId !== authUser(request).id || token.revision !== workspace.matchmakingRevision || !validExpiry || !validMode || !validStrength || !validTeams || !validKey || (!adjusted && lineupChanged) || (overrideToManual && !lineupChanged)) throw conflict(overrideToManual && !lineupChanged ? "VALIDATION_ERROR" : "SUGGESTION_STALE", overrideToManual && !lineupChanged ? "Manual conversion requires an edited suggestion." : "Generate a new suggestion.");
  return token;
}
...
async function bumpMatchmakingRevision(queueMasterId: string, tx: any = db) { await tx.queueWorkspace.update({ where: { queueMasterId }, data: { matchmakingRevision: { increment: 1 }, version: { increment: 1 } } }); }
...
async function matchmakingHistoryFor(queueMasterId: string) { return db.match.findMany({ where: { queueMasterId, status: MatchStatus.COMPLETED }, select: { completedAt: true, participants: { select: { id: true, queuePlayerId: true, team: true } } }, orderBy: { completedAt: "desc" }, take: 1000 }); }
async function historyMaps(queueMasterId: string): Promise<MatchHistory> {
  const history = await matchmakingHistoryFor(queueMasterId);
...
function feePlayerStatus(due: number, collected: number, waived: number) {
  const outstanding = Math.max(0, due - collected - waived);
  const credit = Math.max(0, collected - Math.max(0, due - waived));
  return credit > 0 ? "CREDIT" : outstanding === 0 && waived > 0 ? "WAIVED" : outstanding === 0 ? "PAID" : collected > 0 ? "PARTIAL" : "UNPAID";
}
async function feeSummary(queueMasterId: string) {
  const [configRecord, workspace, players, payments] = await Promise.all([
    db.queueFeeConfig.findUnique({ where: { queueMasterId } }),
    db.queueWorkspace.findUnique({ where: { queueMasterId }, select: { endedAt: true } }),
    db.queuePlayer.findMany({ where: { queueMasterId }, orderBy: { displayNameSnapshot: "asc" } }),
    db.payment.findMany({ where: { queueMasterId }, orderBy: { occurredAt: "desc" } }),
  ]);
  const totals = collectionTotalsByPlayer(payments);
  const byPlayer = players.map((player: any) => {
    const collected = payments.filter((payment: any) => payment.queuePlayerId === player.id && payment.kind === PaymentKind.COLLECTION).reduce((sum: number, payment: any) => sum + payment.amountMinor, 0);
    const waived = payments.filter((payment: any) => payment.queuePlayerId === player.id && payment.kind === PaymentKind.WAIVER).reduce((sum: number, payment: any) => sum + payment.amountMinor, 0);
    const due = player.amountDueMinor ?? 0;
    const outstanding = Math.max(0, due - collected - waived);
    const credit = Math.max(0, collected - Math.max(0, due - waived));
    const methodTotals = totals.get(player.id) ?? { CASH: 0, EWALLET: 0, OTHER: 0 };
    return { queuePlayerId: player.id, displayName: player.displayNameSnapshot, dueMinor: due, collectedMinor: collected, waivedMinor: waived, outstandingMinor: outstanding, creditMinor: credit, isNoShow: Boolean(workspace?.endedAt && player.matchesPlayed === 0), status: feePlayerStatus(due, collected, waived), collectionByMethodMinor: methodTotals };
  });
  return { config: configRecord, expectedMinor: byPlayer.reduce((sum: number, row: any) => sum + row.dueMinor, 0), collectedMinor: byPlayer.reduce((sum: number, row: any) => sum + row.collectedMinor, 0), outstandingMinor: byPlayer.reduce((sum: number, row: any) => sum + row.outstandingMinor, 0), creditMinor: byPlayer.reduce((sum: number, row: any) => sum + row.creditMinor, 0), noShowCount: byPlayer.filter((row: any) => row.isNoShow).length, paymentCount: payments.length, players: byPlayer };
}
...
api.patch("/settings", requireAuth, requireMutationOrigin, route(async (request, response) => {
  const queueMasterId = authUser(request).id;
  const { settings: current } = await ensureWorkspace(queueMasterId);
  assertVersion(current.version, versionFrom(request));
  const body = parse(z.object({ pointsToWin: z.number().int().min(1).max(99).optional(), winBy: z.number().int().min(1).max(10).optional(), scoreCap: z.number().int().min(1).max(99).nullable().optional(), bestOf: z.union([z.literal(1), z.literal(3)]).optional(), minimumRestMinutes: z.number().int().min(0).max(60).optional(), lateArrivalGraceMinutes: z.number().int().min(1).max(60).optional(), defaultFeeMode: z.enum([FeeMode.FIXED_PER_PLAYER, FeeMode.EQUAL_SPLIT]).optional(), defaultFixedFeeMinor: z.number().int().min(0).max(2_000_000_000).nullable().optional(), noShowPenaltyMinor: z.number().int().min(0).max(2_000_000_000).optional(), defaultLateArrivalCutoffTime: clockTimeSchema.nullable().optional() }), request.body);
  const points = body.pointsToWin ?? current.pointsToWin;
  const scoreCap = body.scoreCap === undefined ? current.scoreCap : body.scoreCap;
  if (scoreCap !== null && scoreCap < points) throw badRequest("The score cap cannot be lower than points to win.");
  const updated = await db.$transaction(async (tx: any) => {
    const next = await tx.queueMasterSettings.update({ where: { id: current.id }, data: { ...body, pointsToWin: points, scoreCap, version: { increment: 1 } } });
    const liveWorkspace = await tx.queueWorkspace.findUnique({ where: { queueMasterId }, select: { endedAt: true } });
    if (!liveWorkspace?.endedAt && body.noShowPenaltyMinor !== undefined) await tx.queueFeeConfig.update({ where: { queueMasterId }, data: { noShowPenaltyMinor: body.noShowPenaltyMinor, version: { increment: 1 } } });
    return next;
  });
  if (body.minimumRestMinutes !== undefined && body.minimumRestMinutes !== current.minimumRestMinutes) await bumpMatchmakingRevision(queueMasterId);
  responseData(response, settingsView(updated));
}));
...
api.patch("/workspace/late-arrival-policy", requireAuth, requireMutationOrigin, route(async (request, response) => {
  ...
  const body = parse(z.discriminatedUnion("mode", [z.object({ mode: z.literal("SET_NOW") }), z.object({ mode: z.literal("START_GRACE"), graceMinutes: z.number().int().min(1).max(60).optional(), settingsVersion: z.number().int().min(1).optional() }), z.object({ mode: z.literal("APPLY_ACCOUNT_DEFAULT") }), z.object({ mode: z.literal("SET_CUSTOM"), localDateTime: z.string().regex(/^\d{4}-\d{2}-\d{2}T(?:[01]\d|2[0-3]):[0-5]\d$/) }), z.object({ mode: z.literal("DISABLED") })]), request.body);
  const graceMinutes = body.mode === "START_GRACE" ? body.graceMinutes ?? settings.lateArrivalGraceMinutes ?? DEFAULT_LATE_ARRIVAL_GRACE_MINUTES : null;
  let cutoff: Date | null = null;
  try {
    if (body.mode === "SET_NOW") cutoff = new Date();
    else if (body.mode === "START_GRACE") cutoff = new Date(Date.now() + graceMinutes * 60_000);
  ...
  const updated = await withTransactionRetry(async (tx) => {
    if (body.mode === "START_GRACE" && body.graceMinutes !== undefined) {
      if (body.settingsVersion === undefined) await tx.queueMasterSettings.update({ where: { queueMasterId }, data: { lateArrivalGraceMinutes: body.graceMinutes, version: { increment: 1 } } });
      else {
        const saved = await tx.queueMasterSettings.updateMany({ where: { queueMasterId, version: body.settingsVersion }, data: { lateArrivalGraceMinutes: body.graceMinutes, version: { increment: 1 } } });
        if (!saved.count) throw conflict("VERSION_CONFLICT", "The account settings changed on another device.");
      }
    }
...
async function assertSynergyTeamMembersAvailable(tx: any, queueMasterId: string, ids: string[], excludeTeamId?: string) {
  if (ids.length !== 2 || ids[0] === ids[1]) throw conflict("INVALID_SYNERGY_TEAM", "Choose exactly two distinct queue players.");
  const players = await tx.queuePlayer.findMany({ where: { queueMasterId, id: { in: ids } } });
  if (players.length !== 2) throw conflict("PLAYER_NOT_FOUND", "Both Synergy Team members must belong to this session.");
  const busy = players.filter((player: any) => [QueuePlayerStatus.QUEUED, QueuePlayerStatus.PLAYING].includes(player.status));
  if (busy.length) throw conflict("PLAYER_BUSY", "Synergy Teams can only be changed while both players are free.", { queuePlayerIds: busy.map((player: any) => player.id) });
  const overlaps = await tx.synergyTeam.findMany({ where: { queueMasterId, ...(excludeTeamId ? { id: { not: excludeTeamId } } : {}) } });
  if (overlaps.some((team: any) => team.queuePlayerIds.some((id: string) => ids.includes(id)))) throw conflict("SYNERGY_TEAM_CONFLICT", "Each queue player can belong to only one Synergy Team.");
  return players;
}
api.post("/queue/synergy-teams", requireAuth, requireMutationOrigin, route(async (request, response) => {
  const queueMasterId = authUser(request).id;
  await activeWorkspaceFor(request);
  const body = parse(z.object({ queuePlayerIds: z.array(idSchema).length(2) }), request.body);
  const team = await withTransactionRetry(async (tx: any) => {
    const players = await assertSynergyTeamMembersAvailable(tx, queueMasterId, body.queuePlayerIds);
    const created = await tx.synergyTeam.create({ data: { queueMasterId, queuePlayerIds: body.queuePlayerIds, version: 1 } });
    await tx.queueWorkspace.update({ where: { queueMasterId }, data: { matchmakingRevision: { increment: 1 }, version: { increment: 1 } } });
    await audit(tx, request, { action: "SYNERGY_TEAM_CREATED", entityType: "SYNERGY_TEAM", entityId: created.id, reason: "Locked two queue players together for this session", after: { ...created, queuePlayerIds: body.queuePlayerIds, memberNames: players.map((player: any) => player.displayNameSnapshot) } });
    return created;
  });
  const players = await db.queuePlayer.findMany({ where: { queueMasterId } });
  responseData(response, synergyTeamView(team, new Map<string, any>(players.map((player: any) => [player.id, player] as [string, any]))), 201);
}));
api.patch("/queue/synergy-teams/:id", requireAuth, requireMutationOrigin, route(async (request, response) => {
  const queueMasterId = authUser(request).id;
  await activeWorkspaceFor(request);
  const body = parse(z.object({ queuePlayerIds: z.array(idSchema).length(2) }), request.body);
  const current = await db.synergyTeam.findFirst({ where: { id: String(request.params.id), queueMasterId } });
  if (!current) throw notFound("Synergy Team not found.");
  assertVersion(current.version, versionFrom(request));
  const team = await withTransactionRetry(async (tx: any) => {
    const latest = await tx.synergyTeam.findFirst({ where: { id: current.id, queueMasterId } });
    if (!latest) throw notFound("Synergy Team not found.");
    assertVersion(latest.version, versionFrom(request));
    await assertSynergyTeamMembersAvailable(tx, queueMasterId, latest.queuePlayerIds, latest.id);
    await assertSynergyTeamMembersAvailable(tx, queueMasterId, body.queuePlayerIds, latest.id);
    const updated = await tx.synergyTeam.update({ where: { id: latest.id }, data: { queuePlayerIds: body.queuePlayerIds, version: { increment: 1 } } });
    await tx.queueWorkspace.update({ where: { queueMasterId }, data: { matchmakingRevision: { increment: 1 }, version: { increment: 1 } } });
    await audit(tx, request, { action: "SYNERGY_TEAM_UPDATED", entityType: "SYNERGY_TEAM", entityId: updated.id, reason: "Updated locked queue-player members", before: latest, after: updated });
    return updated;
  });
  const players = await db.queuePlayer.findMany({ where: { queueMasterId } });
  responseData(response, synergyTeamView(team, new Map<string, any>(players.map((player: any) => [player.id, player] as [string, any]))));
}));
api.delete("/queue/synergy-teams/:id", requireAuth, requireMutationOrigin, route(async (request, response) => {
  const queueMasterId = authUser(request).id;
  await activeWorkspaceFor(request);
  const current = await db.synergyTeam.findFirst({ where: { id: String(request.params.id), queueMasterId } });
  if (!current) throw notFound("Synergy Team not found.");
  ...
  await withTransactionRetry(async (tx: any) => {
    ...
    assertVersion(latest.version, versionFrom(request));
    await assertSynergyTeamMembersAvailable(tx, queueMasterId, latest.queuePlayerIds, latest.id);
    await tx.synergyTeam.delete({ where: { id: latest.id } });
    await tx.queueWorkspace.update({ where: { queueMasterId }, data: { matchmakingRevision: { increment: 1 }, version: { increment: 1 } } });
    await audit(tx, request, { action: "SYNERGY_TEAM_DISSOLVED", entityType: "SYNERGY_TEAM", entityId: latest.id, reason: "Dissolved locked queue-player members", before: latest });
  });
  noContent(response);
}));
api.delete("/queue/players/:id", requireAuth, requireMutationOrigin, route(async (request, response) => {
  ...
  await withTransactionRetry(async (tx: any) => {
    ...
    await tx.synergyTeam.deleteMany({ where: { queueMasterId, queuePlayerIds: { has: player.id } } });
...
api.post("/queue/players/bulk-action", requireAuth, requireMutationOrigin, route(async (request, response) => {
  ...
  responseData(response, updated.map((player: any) => queuePlayerView(player)));
}));
...
api.post("/queue/players/:id/rest", requireAuth, requireMutationOrigin, route(async (request, response) => { const player = await ownedQueuePlayer(request, request.params.id); if (player.status !== QueuePlayerStatus.WAITING) throw conflict("INVALID_PLAYER_TRANSITION", "Only waiting players can rest."); const updated = await db.queuePlayer.update({ where: { id: player.id }, data: { status: QueuePlayerStatus.RESTING, restStartedAt: new Date(), version: { increment: 1 } } }); await bumpMatchmakingRevision(authUser(request).id); responseData(response, queuePlayerView(updated)); }));
...
api.post("/suggestions", requireAuth, route(async (request, response) => { const parsed = parse(suggestionRequestSchema, request.body); if (parsed.mode === MatchmakingMode.GUIDED) { await guidedSuggestions(request, response); return; } const body = parsed; const matchmakingStartedAt = Date.now(); const workspace = await workspaceFor(request); const settings = (await ensureWorkspace(authUser(request).id)).settings; const [players, history, synergyTeams] = await Promise.all([db.queuePlayer.findMany({ where: { queueMasterId: authUser(request).id }, select: { id: true, displayNameSnapshot: true, genderSnapshot: true, skillLevelSnapshot: true, status: true, matchesPlayed: true, wins: true, losses: true, queueEnteredAt: true, lastMatchEndedAt: true, manualPriority: true, latePenaltyState: true, latePenaltyAppliedAt: true } }), historyMaps(authUser(request).id), loadSynergyTeams(authUser(request).id)]); const strengthGap = body.mode === MatchmakingMode.BALANCED ? body.strengthGap ?? 1 : undefined; const options: MatchmakingOptions = { strengthGap, minimumRestMinutes: settings.minimumRestMinutes, now: new Date(), synergyTeams: synergyTeams.map((team) => ({ id: team.id, queuePlayerIds: team.queuePlayerIds as [string, string], createdAt: team.createdAt.toISOString(), version: team.version })) }; const input: MatchPlayer[] = players.map((p: any) => ({ id: p.id, displayName: p.displayNameSnapshot, gender: p.genderSnapshot, skillWeight: skillWeight(p.skillLevelSnapshot as SkillLevel), skillLevel: p.skillLevelSnapshot, status: p.status, gamesPlayed: p.matchesPlayed, wins: p.wins, losses: p.losses, queueEnteredAt: p.queueEnteredAt, lastMatchEndedAt: p.lastMatchEndedAt, manualPriority: p.manualPriority, latePenaltyState: p.latePenaltyState, latePenaltyAppliedAt: p.latePenaltyAppliedAt })); const eligibleCount = input.filter((player) => player.status === QueuePlayerStatus.WAITING && restEligibleAt(player.lastMatchEndedAt, settings.minimumRestMinutes).getTime() <= Date.now()).length; let suggestion = suggestMatch(input, body.mode as MatchmakingMode, history, body.excludeKeys, options); let cycleRestarted = false; if (!suggestion && body.excludeKeys.length && body.mode !== MatchmakingMode.UNDEFEATED_CHALLENGE) { suggestion = suggestMatch(input, body.mode as MatchmakingMode, history, [], options); cycleRestarted = Boolean(suggestion); } if (!suggestion) { const now = Date.now(); const challengeRanked = undefeatedChallengePlayers(input); const challengeReady = challengeRanked.filter(({ player }) => player.status === QueuePlayerStatus.WAITING && restEligibleAt(player.lastMatchEndedAt, settings.minimumRestMinutes, now).getTime() <= now); const waiting = players.filter((player: any) => player.status === QueuePlayerStatus.WAITING); const ready = waiting.filter((player: any) => restEligibleAt(player.lastMatchEndedAt, settings.minimumRestMinutes, now).getTime() <= now); const nextEligibleAt = waiting.map((player: any) => restEligibleAt(player.lastMatchEndedAt, settings.minimumRestMinutes, now).getTime()).filter((value: number) => value > now).sort((a: number, b: number) => a - b)[0]; const waitingMaleCount = waiting.filter((player: any) => player.genderSnapshot === Gender.MALE).length; const waitingFemaleCount = waiting.filter((player: any) => player.genderSnapshot === Gender.FEMALE).length; const readyMaleCount = ready.filter((player: any) => player.genderSnapshot === Gender.MALE).length; const readyFemaleCount = ready.filter((player: any) => player.genderSnapshot === Gender.FEMALE).length; const compositionPool = players.filter((player: any) => player.status === QueuePlayerStatus.WAITING || player.status === QueuePlayerStatus.RESTING); const compositionMaleCount = compositionPool.filter((player: any) => player.genderSnapshot === Gender.MALE).length; const compositionFemaleCount = compositionPool.filter((player: any) => player.genderSnapshot === Gender.FEMALE).length; const mixedCompositionBlocked = body.mode === MatchmakingMode.MIXED_DOUBLES && (compositionMaleCount < 2 || compositionFemaleCount < 2); const restBlocked = !mixedCompositionBlocked && ((body.mode === MatchmakingMode.MIXED_DOUBLES && (readyMaleCount < 2 || readyFemaleCount < 2)) || (body.mode !== MatchmakingMode.MIXED_DOUBLES && waiting.length >= 4 && ready.length < 4)) && settings.minimumRestMinutes > 0; const challengeNoMatch = body.mode === MatchmakingMode.UNDEFEATED_CHALLENGE; const challengeRestBlocked = challengeNoMatch && restBlocked; const noChallengeAlternate = challengeNoMatch && body.excludeKeys.length > 0; const message = challengeNoMatch ? (challengeRanked.length === 0 ? `No top-three player has reached ${UNDEFEATED_CHALLENGE_MINIMUM_MATCHES} wins without a loss yet.` : challengeReady.length === 0 ? "Qualified players are not ready for an Undefeated Challenge match." : challengeRestBlocked ? "Some waiting players are still completing their required rest period." : noChallengeAlternate ? "No other valid Undefeated Challenge lineup keeps the undefeated player at a disadvantage." : "No valid Undefeated Challenge lineup is available under the challenge rules.") : mixedCompositionBlocked ? "Mixed doubles requires exactly two ready male and two ready female players." : restBlocked ? "Some players are still completing their required rest period." : body.mode === MatchmakingMode.BALANCED ? `No exact Handicap +${strengthGap} strength lineup is available. Choose a tighter mode and generate again.` : "No eligible group satisfies this mode."; logger.info({ eligibleCount: eligibleCount, evaluatedCount: Math.min(eligibleCount, 40), boundedSearch: eligibleCount > 40, mode: body.mode, resultCode: challengeNoMatch ? (challengeRanked.length === 0 ? "NO_UNDEFEATED_QUALIFIER" : challengeReady.length === 0 || challengeRestBlocked ? "REST_REQUIRED" : "NO_VALID_GROUP") : mixedCompositionBlocked ? "NO_MIXED_DOUBLES_COMPOSITION" : restBlocked ? "REST_REQUIRED" : body.mode === MatchmakingMode.BALANCED ? "NO_EXACT_STRENGTH_GAP" : "NO_VALID_GROUP", durationMs: Date.now() - matchmakingStartedAt }, "matchmaking suggestion"); responseData(response, { suggestion: null, cycleRestarted: false, noMatch: { code: challengeNoMatch ? (challengeRanked.length === 0 ? "NO_UNDEFEATED_QUALIFIER" : challengeReady.length === 0 || challengeRestBlocked ? "REST_REQUIRED" : "NO_VALID_GROUP") : mixedCompositionBlocked ? "NO_MIXED_DOUBLES_COMPOSITION" : restBlocked ? "REST_REQUIRED" : body.mode === MatchmakingMode.BALANCED ? "NO_EXACT_STRENGTH_GAP" : "NO_VALID_GROUP", message, nextEligibleAt: nextEligibleAt ? new Date(nextEligibleAt) : null, readyMaleCount, readyFemaleCount, waitingMaleCount, waitingFemaleCount, ...(challengeNoMatch ? { challenge: challengeNoMatchDetails(input, settings.minimumRestMinutes, body.excludeKeys) } : {}) } }); return; } const expiresAt = Date.now() + 300_000; const token = signSuggestion({ queueMasterId: authUser(request).id, revision: workspace.matchmakingRevision, mode: body.mode, strengthGap, key: suggestion.key, teamA: suggestion.teamA.map((p) => p.id), teamB: suggestion.teamB.map((p) => p.id), expiresAt }); const viewPlayer = (p: MatchPlayer) => ({ id: p.id, displayName: p.displayName, gender: p.gender, skillLevel: p.skillLevel, skillWeight: p.skillWeight, synergyTeamId: p.synergyTeamId ?? null, effectiveSkillLevel: p.effectiveSkillLevel ?? p.skillLevel, effectiveSkillWeight: p.effectiveSkillWeight ?? p.skillWeight, gamesPlayed: p.gamesPlayed, wins: p.wins, losses: p.losses, queueEnteredAt: p.queueEnteredAt, lastMatchEndedAt: p.lastMatchEndedAt, manualPriority: p.manualPriority, latePenaltyState: p.latePenaltyState, latePenaltyAppliedAt: p.latePenaltyAppliedAt }); logger.info({ eligibleCount: eligibleCount, evaluatedCount: Number((suggestion.explanation as any).searchStats?.evaluatedCount ?? input.length), boundedSearch: Boolean((suggestion.explanation as any).searchStats?.bounded), mode: body.mode, resultCode: "MATCH_FOUND", durationMs: Date.now() - matchmakingStartedAt }, "matchmaking suggestion"); responseData(response, { cycleRestarted, suggestion: { token, expiresAt, lateArrivalCutoffAt: workspace.lateArrivalCutoffAt, strengthGap, ...suggestion, teamA: suggestion.teamA.map(viewPlayer), teamB: suggestion.teamB.map(viewPlayer), explanation: { ...suggestion.explanation, algorithmVersion: MATCHMAKING_ALGORITHM, generatedGenderRule: "NO_TWO_FEMALE_VS_TWO_MALE", cycleRestarted } } }); }));

async function guidedSuggestions(request: Request, response: Response) {
  const body = parse(z.strictObject({ mode: z.literal(MatchmakingMode.GUIDED), excludeKeys: z.array(z.string()).max(50).default([]) }), request.body);
  const matchmakingStartedAt = Date.now();
  const workspace = await workspaceFor(request);
  const settings = (await ensureWorkspace(authUser(request).id)).settings;
  const [players, history, synergyTeams] = await Promise.all([
    db.queuePlayer.findMany({ where: { queueMasterId: authUser(request).id }, select: { id: true, displayNameSnapshot: true, genderSnapshot: true, skillLevelSnapshot: true, status: true, matchesPlayed: true, wins: true, losses: true, queueEnteredAt: true, lastMatchEndedAt: true, manualPriority: true, latePenaltyState: true, latePenaltyAppliedAt: true } }),
    historyMaps(authUser(request).id),
    loadSynergyTeams(authUser(request).id),
  ]);
  const options: MatchmakingOptions = { minimumRestMinutes: settings.minimumRestMinutes, now: new Date(), synergyTeams: synergyTeams.map((team) => ({ id: team.id, queuePlayerIds: team.queuePlayerIds as [string, string], createdAt: team.createdAt.toISOString(), version: team.version })) };
  const input: MatchPlayer[] = players.map((player: any) => ({ id: player.id, displayName: player.displayNameSnapshot, gender: player.genderSnapshot, skillWeight: skillWeight(player.skillLevelSnapshot as SkillLevel), skillLevel: player.skillLevelSnapshot, status: player.status, gamesPlayed: player.matchesPlayed, wins: player.wins, losses: player.losses, queueEnteredAt: player.queueEnteredAt, lastMatchEndedAt: player.lastMatchEndedAt, manualPriority: player.manualPriority, latePenaltyState: player.latePenaltyState, latePenaltyAppliedAt: player.latePenaltyAppliedAt }));
  let suggestion = suggestMatch(input, MatchmakingMode.GUIDED, history, body.excludeKeys, options);
  let cycleRestarted = false;
  if (!suggestion && body.excludeKeys.length) {
    suggestion = suggestMatch(input, MatchmakingMode.GUIDED, history, [], options);
    cycleRestarted = Boolean(suggestion);
  }
  if (!suggestion) {
    const status = guidedAvailabilityStatus(input, settings.minimumRestMinutes, synergyTeams);
    if (status.available && body.excludeKeys.length === 0) logger.warn({ mode: MatchmakingMode.GUIDED, waitingLearnerCount: status.waitingLearnerCount, waitingGuideCount: status.waitingGuideCount, readyLearnerCount: status.readyLearnerCount, readyGuideCount: status.readyGuideCount, durationMs: Date.now() - matchmakingStartedAt }, "guided availability invariant violated");
    const code = status.reason ?? "NO_VALID_GROUP";
    const message = code === "NO_GUIDED_COMPOSITION"
      ? `Guided requires two waiting Newbie/Beginner learners and two waiting Intermediate guides (${status.readyLearnerCount}/${status.waitingLearnerCount} learners ready, ${status.readyGuideCount}/${status.waitingGuideCount} guides ready).`
      : code === "REST_REQUIRED"
        ? `Guided has enough learners and guides, but some are still resting (${status.readyLearnerCount}/${status.waitingLearnerCount} learners ready, ${status.readyGuideCount}/${status.waitingGuideCount} guides ready).`
        : "No valid Guided partition satisfies the gender or Synergy Team constraints.";
    responseData(response, { suggestion: null, cycleRestarted: false, noMatch: { code, message, nextEligibleAt: status.nextEligibleAt, waitingLearnerCount: status.waitingLearnerCount, waitingGuideCount: status.waitingGuideCount, readyLearnerCount: status.readyLearnerCount, readyGuideCount: status.readyGuideCount } });
    logger.info({ mode: MatchmakingMode.GUIDED, resultCode: code, waitingLearnerCount: status.waitingLearnerCount, waitingGuideCount: status.waitingGuideCount, readyLearnerCount: status.readyLearnerCount, readyGuideCount: status.readyGuideCount, durationMs: Date.now() - matchmakingStartedAt }, "matchmaking suggestion");
    return;
  }
  const expiresAt = Date.now() + 300_000;
  const token = signSuggestion({ queueMasterId: authUser(request).id, revision: workspace?.matchmakingRevision, mode: MatchmakingMode.GUIDED, key: suggestion.key, teamA: suggestion.teamA.map((player) => player.id), teamB: suggestion.teamB.map((player) => player.id), expiresAt });
  const viewPlayer = (player: MatchPlayer) => ({ id: player.id, displayName: player.displayName, gender: player.gender, skillLevel: player.skillLevel, skillWeight: player.skillWeight, synergyTeamId: player.synergyTeamId ?? null, effectiveSkillLevel: player.effectiveSkillLevel ?? player.skillLevel, effectiveSkillWeight: player.effectiveSkillWeight ?? player.skillWeight, gamesPlayed: player.gamesPlayed, wins: player.wins, losses: player.losses, queueEnteredAt: player.queueEnteredAt, lastMatchEndedAt: player.lastMatchEndedAt, manualPriority: player.manualPriority, latePenaltyState: player.latePenaltyState, latePenaltyAppliedAt: player.latePenaltyAppliedAt });
  responseData(response, { cycleRestarted, suggestion: { token, expiresAt, lateArrivalCutoffAt: workspace?.lateArrivalCutoffAt ?? null, strengthGap: undefined, ...suggestion, teamA: suggestion.teamA.map(viewPlayer), teamB: suggestion.teamB.map(viewPlayer), explanation: { ...suggestion.explanation, algorithmVersion: MATCHMAKING_ALGORITHM, generatedGenderRule: "NO_TWO_FEMALE_VS_TWO_MALE", cycleRestarted } } });
  logger.info({ mode: MatchmakingMode.GUIDED, resultCode: "MATCH_FOUND", evaluatedCount: Number((suggestion.explanation as any).searchStats?.evaluatedCount ?? 0), boundedSearch: Boolean((suggestion.explanation as any).searchStats?.bounded), waitingLearnerCount: input.filter((player) => ["NEWBIE", "BEGINNER"].includes(player.skillLevel) && player.status === QueuePlayerStatus.WAITING).length, waitingGuideCount: input.filter((player) => player.skillLevel === "INTERMEDIATE" && player.status === QueuePlayerStatus.WAITING).length, durationMs: Date.now() - matchmakingStartedAt }, "matchmaking suggestion");
}

async function createMatch(request: Request, body: { teamA: string[]; teamB: string[]; courtId?: string; suggestionToken?: string; suggestionAdjusted?: boolean; overrideToManual?: boolean; playerPreferenceConfirmed?: boolean; suggestionPayload?: Record<string, unknown> }) {
  ...
  const matchId = await withTransactionRetry(async (tx) => {
    ...
    let generatedMatchupAdvisory: ReturnType<typeof lowSkillLoneFemaleAdvisory> = null;
    if (body.suggestionPayload && !body.overrideToManual) {
    ...
    const synergyTeams = (await tx.synergyTeam.findMany({ where: { queueMasterId } })).filter(validSynergyTeam);
    const byId = new Map<string, any>(players.map((player: any) => [player.id, player] as [string, any]));
    const toMatchPlayer = (id: string): MatchPlayer => matchmakingPlayerFromQueuePlayer(byId.get(id), byId, synergyTeams);
    const teamAPlayers = body.teamA.map(toMatchPlayer);
    const teamBPlayers = body.teamB.map(toMatchPlayer);
    const challengeSource = body.suggestionPayload?.mode === MatchmakingMode.UNDEFEATED_CHALLENGE
      ? await tx.queuePlayer.findMany({ where: { queueMasterId } })
      : players;
    const challengeQualifiedIds = body.suggestionPayload?.mode === MatchmakingMode.UNDEFEATED_CHALLENGE
      ? new Set(undefeatedChallengePlayers(challengeSource.map((player: any) => challengeInput(player))).map(({ player }) => player.id))
      : new Set<string>();
    const challengeSelectedPlayerIds = all.filter((id) => challengeQualifiedIds.has(id));
    const synergyError = validateSynergyLineup(teamAPlayers, teamBPlayers, synergyTeams as any);
    if (body.suggestionPayload?.mode === MatchmakingMode.GUIDED && !body.overrideToManual) validateGuidedMutation(teamAPlayers, teamBPlayers, synergyError);
    else if (synergyError) throw conflict("SYNERGY_TEAM_LINEUP", synergyError);
    generatedMatchupAdvisory = lowSkillLoneFemaleAdvisory(teamAPlayers, teamBPlayers);
    if (body.suggestionPayload && !body.overrideToManual) {
      const suggestionSynergyError = validateSynergyLineup(teamAPlayers, teamBPlayers, synergyTeams as any);
      if (body.suggestionPayload.mode !== MatchmakingMode.GUIDED && suggestionSynergyError) throw conflict("SYNERGY_TEAM_LINEUP", suggestionSynergyError);
      ...
      generatedMatchupAdvisory = lowSkillLoneFemaleAdvisory(teamAPlayers, teamBPlayers);
      if (body.suggestionPayload.mode === MatchmakingMode.MIXED_DOUBLES) {
        const mixedError = validateMixedDoublesLineup(teamAPlayers, teamBPlayers);
        if (mixedError) throw conflict("MIXED_DOUBLES_COMPOSITION", mixedError, { canConvertToManual: true, originalMode: MatchmakingMode.MIXED_DOUBLES });
      }
      if (isProhibitedGeneratedGenderMatch(teamAPlayers, teamBPlayers)) throw conflict("GENERATED_GENDER_RULE", "Generated matchups cannot place two female players against two male players.", { canConvertToManual: true, originalMode: body.suggestionPayload.mode });
      if (body.suggestionPayload.mode === MatchmakingMode.BALANCED) {
        const strengthGap = Number(body.suggestionPayload.strengthGap ?? 1);
        const balanceError = validateBalancedLineup(teamAPlayers, teamBPlayers, strengthGap);
        if (balanceError) throw conflict("BALANCE_CONSTRAINT_VIOLATION", balanceError, { strengthGap, canConvertToManual: true, originalMode: MatchmakingMode.BALANCED });
      }
      if (body.suggestionPayload.mode === MatchmakingMode.SAME_SKILL || body.suggestionPayload.mode === MatchmakingMode.SAME_GENDER || body.suggestionPayload.mode === MatchmakingMode.OPEN) {
        const guaranteeError = validateModeGuarantee(body.suggestionPayload.mode as MatchmakingMode, teamAPlayers, teamBPlayers, Number(body.suggestionPayload.strengthGap ?? 1));
        if (guaranteeError) throw conflict("MODE_CONSTRAINT_VIOLATION", guaranteeError, { canConvertToManual: true, originalMode: body.suggestionPayload.mode });
      }
      if (body.suggestionPayload.mode === MatchmakingMode.UNDEFEATED_CHALLENGE && body.suggestionAdjusted) throw conflict("UNDEFEATED_CHALLENGE_CONSTRAINT", "Edited Undefeated Challenge lineups must continue as Manual Adjusted.", { canConvertToManual: true, originalMode: MatchmakingMode.UNDEFEATED_CHALLENGE });
    }
    if (body.suggestionPayload && !body.overrideToManual && body.suggestionPayload.mode !== MatchmakingMode.GUIDED && isProhibitedGeneratedNewbieMatch(teamAPlayers, teamBPlayers)) {
      throw conflict("MODE_CONSTRAINT_VIOLATION", "Generated matchups cannot pair a Newbie with an incompatible partner.", { canConvertToManual: true, originalMode: body.suggestionPayload.mode });
    }
    ...
    if (court && generatedMatchupAdvisory && body.playerPreferenceConfirmed !== true) throw conflict("PLAYER_PREFERENCE_CONFIRMATION_REQUIRED", "Confirm that the affected player accepts this matchup before starting.", generatedMatchupAdvisory);
    ...
    const convertedToManual = Boolean(body.overrideToManual);
    const persistedMode = convertedToManual || challengeAdjusted ? null : body.suggestionPayload?.mode as MatchmakingMode | undefined ?? null;
    const persistedAlgorithm = body.suggestionToken && !convertedToManual && !challengeAdjusted ? MATCHMAKING_ALGORITHM : null;
    const suggestionExplanation = body.suggestionPayload ? { algorithmVersion: persistedAlgorithm, strengthGap: body.suggestionPayload.strengthGap ?? null, mode: persistedMode, originalMode: body.suggestionPayload.mode ?? null, challengeRelabeled: challengeAdjusted, adjusted: Boolean(body.suggestionAdjusted), generatedOrigin: "SUGGESTION", generatedGenderRule: "NO_TWO_FEMALE_VS_TWO_MALE", loneFemalePolicy: generatedLoneFemalePolicy, matchupAdvisory: generatedMatchupAdvisory, rest: { minimumRestMinutes: settings.minimumRestMinutes }, ...(body.suggestionPayload.mode === MatchmakingMode.GUIDED ? { guided: buildGuidedExplanation(all.map((id) => { const player = toMatchPlayer(id); return { id: player.id, skillLevel: player.skillLevel }; })) } : {}) } : generatedMatchupAdvisory ? { matchupAdvisory: generatedMatchupAdvisory } : null;
if (body.suggestionPayload?.mode === MatchmakingMode.UNDEFEATED_CHALLENGE && !convertedToManual && !challengeAdjusted && suggestionExplanation && typeof suggestionExplanation === "object") {
      const selectedQualifierIds = new Set(challengeSelectedPlayerIds);
      const qualifierTeam = teamAPlayers.some((player) => selectedQualifierIds.has(player.id)) ? teamAPlayers : teamBPlayers;
      const opponentTeam = qualifierTeam === teamAPlayers ? teamBPlayers : teamAPlayers;
      const qualifierCount = selectedQualifierIds.size;
      const appliedDisadvantage = qualifierCount === 1
        ? opponentTeam.reduce((sum, player) => sum + (player.effectiveSkillWeight ?? player.skillWeight), 0) - qualifierTeam.reduce((sum, player) => sum + (player.effectiveSkillWeight ?? player.skillWeight), 0)
        : null;
      if (!qualifierCount || (appliedDisadvantage !== null && ![0, 1, 2].includes(appliedDisadvantage))) throw conflict("UNDEFEATED_CHALLENGE_CONSTRAINT", "The undefeated player no longer has a legal Challenge lineup. Generate a new challenge or continue as Manual Adjusted.", { canConvertToManual: true, originalMode: MatchmakingMode.UNDEFEATED_CHALLENGE });
      Object.assign(suggestionExplanation, {
        challenge: {
          minimumMatches: UNDEFEATED_CHALLENGE_MINIMUM_MATCHES,
          rankLimit: UNDEFEATED_CHALLENGE_RANK_LIMIT,
          selectedPlayerIds: challengeSelectedPlayerIds,
          opposingPlayerIds: qualifierCount > 1 ? challengeSelectedPlayerIds : [],
          difficultyPolicy: qualifierCount === 1 ? "TEAM_TOTAL_PLUS_ONE_THEN_PLUS_TWO_THEN_EQUAL" : "QUALIFIED_PLAYERS_OPPOSED",
          appliedDisadvantage,
          equalStrengthFallback: qualifierCount === 1 && appliedDisadvantage === 0,
        },
      });
    }
    if (convertedToManual && suggestionExplanation && typeof suggestionExplanation === "object") Object.assign(suggestionExplanation, { originalAlgorithmVersion: body.suggestionPayload?.algorithmVersion ?? MATCHMAKING_ALGORITHM, originalSuggestionKey: body.suggestionPayload?.key ?? null, overrideToManual: true, adjusted: true });
    const created = await tx.match.create({ data: { queueMasterId, courtId: court?.id ?? null, courtIdSnapshot: court?.id ?? null, courtNameSnapshot: court?.name ?? null, status, source: convertedToManual ? MatchSource.MANUAL_ADJUSTED : source, matchmakingMode: persistedMode, algorithmVersion: persistedAlgorithm, suggestionKey: convertedToManual || challengeAdjusted ? null : typeof body.suggestionPayload?.key === "string" ? body.suggestionPayload.key : null, suggestionExplanation, pointsToWin: settings.pointsToWin, winBy: settings.winBy, scoreCap: settings.scoreCap, bestOf: settings.bestOf, startedAt: court ? new Date() : null, participants: { create: all.map((id) => ({ queuePlayerId: id, priorQueueEnteredAt: players.find((player: any) => player.id === id)?.queueEnteredAt ?? null, team: body.teamA.includes(id) ? TeamSide.A : TeamSide.B, teamSlot: body.teamA.includes(id) ? body.teamA.indexOf(id) + 1 : body.teamB.indexOf(id) + 1 })) } } });
    if (court) {
...
api.post("/matches/:id/start", requireAuth, requireMutationOrigin, route(async (request, response) => {
  const body = parse(z.object({ courtId: idSchema, playerPreferenceConfirmed: z.boolean().optional() }), request.body);
  const queueMasterId = authUser(request).id;
  const settings = (await ensureWorkspace(queueMasterId)).settings;
  const startedId = await withTransactionRetry(async (tx) => {
    ...
    const startById = new Map<string, any>(players.map((player: any) => [player.id, player] as [string, any]));
    const currentSynergyTeams = (await tx.synergyTeam.findMany({ where: { queueMasterId } })).filter(validSynergyTeam);
    const startToMatchPlayer = (id: string): MatchPlayer => matchmakingPlayerFromQueuePlayer(startById.get(id), startById, currentSynergyTeams);
    const startTeamA = match.participants.filter((participant: any) => participant.team === TeamSide.A).sort((a: any, b: any) => a.teamSlot - b.teamSlot).map((participant: any) => startToMatchPlayer(participant.queuePlayerId));
    const startTeamB = match.participants.filter((participant: any) => participant.team === TeamSide.B).sort((a: any, b: any) => a.teamSlot - b.teamSlot).map((participant: any) => startToMatchPlayer(participant.queuePlayerId));
    ...
    if (match.matchmakingMode === MatchmakingMode.UNDEFEATED_CHALLENGE) {
      const currentQueuePlayers = await tx.queuePlayer.findMany({ where: { queueMasterId } });
      validateUndefeatedChallengeStart(match, currentQueuePlayers);
    }

    const startSynergyError = validateSynergyLineup(startTeamA, startTeamB, currentSynergyTeams as any);
    if (match.matchmakingMode === MatchmakingMode.GUIDED) validateGuidedMutation(startTeamA, startTeamB, startSynergyError);
    else if (startSynergyError) throw conflict("SYNERGY_TEAM_LINEUP", startSynergyError);
    const startAdvisory = lowSkillLoneFemaleAdvisory(startTeamA, startTeamB);
    if (startAdvisory && body.playerPreferenceConfirmed !== true) throw conflict("PLAYER_PREFERENCE_CONFIRMATION_REQUIRED", "Confirm that the affected player accepts this matchup before starting.", startAdvisory);
    if (match.matchmakingMode === MatchmakingMode.BALANCED || match.matchmakingMode === MatchmakingMode.MIXED_DOUBLES || match.matchmakingMode === MatchmakingMode.GUIDED || match.matchmakingMode === MatchmakingMode.SAME_SKILL || match.matchmakingMode === MatchmakingMode.SAME_GENDER || match.matchmakingMode === MatchmakingMode.OPEN || match.matchmakingMode === MatchmakingMode.UNDEFEATED_CHALLENGE) {
      const byId = new Map(players.map((player: any) => [player.id, player]));
       const toMatchPlayer = (id: string): MatchPlayer => matchmakingPlayerFromQueuePlayer(byId.get(id), byId, currentSynergyTeams);
      const teamA = match.participants.filter((participant: any) => participant.team === TeamSide.A).sort((a: any, b: any) => a.teamSlot - b.teamSlot).map((participant: any) => toMatchPlayer(participant.queuePlayerId));
      const teamB = match.participants.filter((participant: any) => participant.team === TeamSide.B).sort((a: any, b: any) => a.teamSlot - b.teamSlot).map((participant: any) => toMatchPlayer(participant.queuePlayerId));
      const lineupSynergyError = validateSynergyLineup(teamA, teamB, currentSynergyTeams as any);
      if (lineupSynergyError && match.matchmakingMode !== MatchmakingMode.GUIDED) throw conflict("SYNERGY_TEAM_LINEUP", lineupSynergyError);
      if (match.matchmakingMode === MatchmakingMode.MIXED_DOUBLES) {
        const mixedError = validateMixedDoublesLineup(teamA, teamB);
        if (mixedError) throw conflict("MIXED_DOUBLES_COMPOSITION", mixedError);
      } else if (match.matchmakingMode !== MatchmakingMode.GUIDED && match.matchmakingMode !== MatchmakingMode.UNDEFEATED_CHALLENGE) {
        const guaranteeError = validateModeGuarantee(match.matchmakingMode as MatchmakingMode, teamA, teamB, Number((match.suggestionExplanation as any)?.strengthGap ?? 1));
        if (guaranteeError) throw conflict(match.matchmakingMode === MatchmakingMode.BALANCED ? "BALANCE_CONSTRAINT_VIOLATION" : "MODE_CONSTRAINT_VIOLATION", guaranteeError, { canConvertToManual: true, originalMode: match.matchmakingMode });
      } else if (match.matchmakingMode === MatchmakingMode.UNDEFEATED_CHALLENGE && match.source === MatchSource.AUTOMATIC) {
        if (isProhibitedGeneratedGenderMatch(teamA, teamB)) throw conflict("GENERATED_GENDER_RULE", "Generated matchups cannot place two female players against two male players.", { canConvertToManual: true, originalMode: match.matchmakingMode });
        if (isProhibitedGeneratedNewbieMatch(teamA, teamB)) throw conflict("MODE_CONSTRAINT_VIOLATION", "Generated matchups cannot pair a Newbie with an incompatible partner.", { canConvertToManual: true, originalMode: match.matchmakingMode });
      }
    }
    ...
    const updated = await tx.match.updateMany({ where: { id: match.id, status: MatchStatus.QUEUED }, data: { courtId: court.id, courtIdSnapshot: court.id, courtNameSnapshot: court.name, ...(startExplanation || startAdvisory ? { suggestionExplanation: { ...(startExplanation ?? {}), matchupAdvisory: startAdvisory } } : {}), status: MatchStatus.IN_PROGRESS, startedAt: new Date(), version: { increment: 1 } } });
...
api.post("/matches/:id/complete", requireAuth, requireMutationOrigin, route(async (request, response) => {
  ...
  const completedView = matchView(await db.match.findUnique({ where: { id: completed.id }, include: { participants: { include: { queuePlayer: true } }, court: true } }));
  const afterChallengePlayers = await db.queuePlayer.findMany({ where: { queueMasterId } });
  responseData(response, { ...completedView, notifications: undefeatedChallengeNotificationEvents(beforeChallengePlayers.map(challengeInput), afterChallengePlayers.map(challengeInput)) });
}));

api.post("/matches", requireAuth, requireMutationOrigin, route(async (request, response) => { const body = parse(z.object({ teamA: z.array(idSchema), teamB: z.array(idSchema), courtId: idSchema.optional(), suggestionToken: z.string().min(20).optional(), suggestionAdjusted: z.boolean().optional(), overrideToManual: z.boolean().optional(), playerPreferenceConfirmed: z.boolean().optional() }), request.body); if (body.overrideToManual && (!body.suggestionToken || !body.suggestionAdjusted)) throw badRequest("Manual conversion requires an edited suggestion."); const suggestionPayload = body.suggestionToken ? await validateSuggestionRequest(request, body.suggestionToken, body.teamA, body.teamB, Boolean(body.suggestionAdjusted), Boolean(body.overrideToManual)) : undefined; responseData(response, matchView(await createMatch(request, { ...body, suggestionPayload })), 201); }));
...
api.post("/matches/start-suggestion", requireAuth, requireMutationOrigin, route(async (request, response) => { const body = parse(z.object({ teamA: z.array(idSchema).length(2), teamB: z.array(idSchema).length(2), courtId: idSchema, suggestionToken: z.string().min(20), suggestionAdjusted: z.boolean().optional(), overrideToManual: z.boolean().optional(), playerPreferenceConfirmed: z.boolean().optional() }), request.body); if (body.overrideToManual && !body.suggestionAdjusted) throw badRequest("Manual conversion requires an edited suggestion."); const suggestionPayload = await validateSuggestionRequest(request, body.suggestionToken, body.teamA, body.teamB, Boolean(body.suggestionAdjusted), Boolean(body.overrideToManual)); responseData(response, matchView(await createMatch(request, { ...body, suggestionToken: body.suggestionToken, suggestionPayload })), 201); }));
...
api.post("/workspace/end", requireAuth, requireMutationOrigin, route(async (request, response) => {
  ...
  const result = await withTransactionRetry(async (tx) => {
    ...
    const checkedInPlayers = await tx.queuePlayer.count({ where: { queueMasterId, checkedInAt: { not: null } } });
    ...
    const roster = await tx.queuePlayer.findMany({ where: { queueMasterId }, select: { id: true, matchesPlayed: true } });
    ...
    const noShowPenaltyMinor = currentConfig?.noShowPenaltyMinor ?? settings?.noShowPenaltyMinor ?? 0;
    const feeConfig = { mode, fixedAmountPerPlayerMinor: mode === FeeMode.FIXED_PER_PLAYER ? currentConfig?.fixedAmountPerPlayerMinor ?? settings?.defaultFixedFeeMinor ?? 0 : null, expectedQueueCostMinor: expectedCost, noShowPenaltyMinor };
    const allocations = allocateFinalFeeAmounts(feeConfig, roster);
    await Promise.all(roster.map((player: any) => tx.queuePlayer.update({ where: { id: player.id }, data: { amountDueMinor: allocations.get(player.id) ?? 0, version: { increment: 1 } } })));
    await tx.queueFeeConfig.upsert({ where: { queueMasterId }, create: { queueMasterId, mode, currencyCode: settings?.currencyCode ?? "PHP", fixedAmountPerPlayerMinor: feeConfig.fixedAmountPerPlayerMinor, expectedQueueCostMinor: expectedCost, noShowPenaltyMinor, frozenAt: endedAt }, update: { noShowPenaltyMinor, frozenAt: endedAt, version: { increment: 1 } } });
    ...
    const noShowCount = roster.filter((player: any) => player.matchesPlayed === 0).length;
    await audit(tx, request, { action: "SESSION_ENDED", entityType: "WORKSPACE", entityId: queueMasterId, reason: "Queue Master ended the session", after: { endedAt, cancelledQueuedMatches: queued.length, checkedInPlayers, rosteredPlayers: roster.length, noShowCount, noShowPenaltyMinor, noShowChargesMinor: noShowCount * noShowPenaltyMinor } });
    return updated;
  });
  const account = await db.queueMaster.findUnique({ where: { id: queueMasterId }, select: { _count: { select: { queuePlayers: true, courts: true } } } });
...
api.patch("/matches/:id", requireAuth, requireMutationOrigin, route(async (request, response) => {
  const queueMasterId = authUser(request).id;
  await activeWorkspaceFor(request);
  const body = parse(z.object({ teamA: z.array(idSchema).min(1).max(2), teamB: z.array(idSchema).min(1).max(2), courtId: idSchema.optional(), swapWithMatchId: idSchema.optional(), overrideToManual: z.boolean().optional() }), request.body);
  ...
  const updatedId = await withTransactionRetry(async (tx) => {
    ...
    if (body.overrideToManual && (!match.matchmakingMode || match.source === MatchSource.MANUAL)) throw badRequest("Manual conversion requires a suggested or generated matchup.");
    ...
    const teamAPlayers = body.teamA.map(toMatchPlayer);
    const teamBPlayers = body.teamB.map(toMatchPlayer);
    const synergyError = validateSynergyLineup(teamAPlayers, teamBPlayers, (await tx.synergyTeam.findMany({ where: { queueMasterId } })).filter(validSynergyTeam) as any);
    if (match.matchmakingMode === MatchmakingMode.GUIDED && !body.overrideToManual) validateGuidedMutation(teamAPlayers, teamBPlayers, synergyError);
    else if (synergyError) throw conflict("SYNERGY_TEAM_LINEUP", synergyError);
    else if (!body.overrideToManual && match.source === MatchSource.AUTOMATIC && isProhibitedGeneratedGenderMatch(teamAPlayers, teamBPlayers)) throw conflict("GENERATED_GENDER_RULE", "Generated matchups cannot place two female players against two male players.", { canConvertToManual: true, originalMode: match.matchmakingMode });
    if (!body.overrideToManual && match.source === MatchSource.AUTOMATIC && match.matchmakingMode !== MatchmakingMode.GUIDED && isProhibitedGeneratedNewbieMatch(teamAPlayers, teamBPlayers)) {
      throw conflict("MODE_CONSTRAINT_VIOLATION", "Generated matchups cannot pair a Newbie with an incompatible partner.", { canConvertToManual: true, originalMode: match.matchmakingMode });
    }
    if (match.matchmakingMode === MatchmakingMode.BALANCED && !body.overrideToManual) {
      ...
      if (balanceError) throw conflict("BALANCE_CONSTRAINT_VIOLATION", balanceError, { strengthGap: Number((match.suggestionExplanation as any)?.strengthGap ?? 1), canConvertToManual: true, originalMode: MatchmakingMode.BALANCED });
    }
    const editedMode = match.matchmakingMode as MatchmakingMode | null;
    if (!body.overrideToManual && editedMode && ([MatchmakingMode.SAME_SKILL, MatchmakingMode.SAME_GENDER, MatchmakingMode.OPEN, MatchmakingMode.MIXED_DOUBLES, MatchmakingMode.UNDEFEATED_CHALLENGE] as MatchmakingMode[]).includes(editedMode)) {
      const guaranteeError = validateModeGuarantee(editedMode, teamAPlayers, teamBPlayers, Number((match.suggestionExplanation as any)?.strengthGap ?? 1));
      if (guaranteeError) throw conflict(editedMode === MatchmakingMode.UNDEFEATED_CHALLENGE ? "UNDEFEATED_CHALLENGE_CONSTRAINT" : editedMode === MatchmakingMode.MIXED_DOUBLES ? "MIXED_DOUBLES_COMPOSITION" : "MODE_CONSTRAINT_VIOLATION", guaranteeError, { canConvertToManual: true, originalMode: editedMode });
    }
