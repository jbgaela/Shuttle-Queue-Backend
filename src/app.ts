...
import { isProhibitedGeneratedGenderMatch, loneFemalePolicy, MATCHMAKING_ALGORITHM, suggestMatch, undefeatedChallengePlayers, validateBalancedLineup, validateMixedDoublesLineup, type MatchHistory, type MatchPlayer, type MatchmakingOptions } from "./lib/matchmaking.js";
...
api.get("/queue", requireAuth, route(async (request, response) => { const workspace = await workspaceFor(request); const settings = (await ensureWorkspace(authUser(request).id)).settings; const players = await db.queuePlayer.findMany({ where: { queueMasterId: authUser(request).id }, orderBy: [{ status: "asc" }, { queueEnteredAt: "asc" }] }); const view = (player: any) => queuePlayerView(player, settings.minimumRestMinutes); responseData(response, { serverTime: new Date(), minimumRestMinutes: settings.minimumRestMinutes, lateArrivalCutoffAt: workspace.lateArrivalCutoffAt, undefeatedChallenge: undefeatedChallengeStatus(players, settings.minimumRestMinutes), inactive: players.filter((p: any) => p.status === QueuePlayerStatus.INACTIVE || p.status === QueuePlayerStatus.CHECKED_OUT).map(view), waiting: players.filter((p: any) => p.status === QueuePlayerStatus.WAITING).map(view), queued: players.filter((p: any) => p.status === QueuePlayerStatus.QUEUED).map(view), playing: players.filter((p: any) => p.status === QueuePlayerStatus.PLAYING).map(view), resting: players.filter((p: any) => p.status === QueuePlayerStatus.RESTING).map(view) }); }));
