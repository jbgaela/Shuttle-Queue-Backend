...
import { buildGuidedExplanation, evaluateGuidedAvailability, isProhibitedGeneratedGenderMatch, loneFemalePolicy, lowSkillLoneFemaleAdvisory, MATCHMAKING_ALGORITHM, suggestMatch, undefeatedChallengePlayers, validateBalancedLineup, validateGuidedLineup, validateMixedDoublesLineup, validateSynergyLineup, type MatchHistory, type MatchPlayer, type MatchmakingOptions } from "./lib/matchmaking.js";
...
function validateGuidedMutation(teamA: MatchPlayer[], teamB: MatchPlayer[], synergyError?: string | null) {
  ...
  if (compositionError) throw conflict("GUIDED_COMPOSITION", compositionError);
  if (synergyError) throw conflict("SYNERGY_TEAM_LINEUP", synergyError);
  if (isProhibitedGeneratedGenderMatch(teamA, teamB)) throw conflict("GENERATED_GENDER_RULE", "Generated matchups cannot place two female players against two male players.");
}
async function validateSuggestionRequest(request: Request, tokenValue: string, teamA: string[], teamB: string[], adjusted: boolean) {
  ...
  const validTeams = Array.isArray(token.teamA) && Array.isArray(token.teamB) && [1, 2].includes(token.teamA.length) && token.teamA.length === token.teamB.length && token.teamA.every((id) => typeof id === "string") && token.teamB.every((id) => typeof id === "string") && new Set([...token.teamA, ...token.teamB]).size === token.teamA.length + token.teamB.length;
  ...
  if (token.algorithmVersion !== MATCHMAKING_ALGORITHM || token.queueMasterId !== authUser(request).id || token.revision !== workspace.matchmakingRevision || !validExpiry || !validMode || !validStrength || !validTeams || !validKey || (!adjusted && (JSON.stringify(token.teamA) !== JSON.stringify(teamA) || JSON.stringify(token.teamB) !== JSON.stringify(teamB)))) throw conflict("SUGGESTION_STALE", "Generate a new suggestion.");
  return token;
}
...
async function createMatch(request: Request, body: { teamA: string[]; teamB: string[]; courtId?: string; suggestionToken?: string; suggestionAdjusted?: boolean; playerPreferenceConfirmed?: boolean; suggestionPayload?: Record<string, unknown> }) {
  ...
  const matchId = await withTransactionRetry(async (tx) => {
    ...
    if (body.suggestionPayload?.mode === MatchmakingMode.GUIDED) validateGuidedMutation(teamAPlayers, teamBPlayers, synergyError);
    ...
    if (body.suggestionPayload) {
      ...
      if (body.suggestionPayload.mode === MatchmakingMode.BALANCED) {
        ...
        if (balanceError) throw conflict("BALANCE_CONSTRAINT_VIOLATION", balanceError, { strengthGap });
      }
    }
...
api.post("/matches/:id/start", requireAuth, requireMutationOrigin, route(async (request, response) => {
  ...
  const startedId = await withTransactionRetry(async (tx) => {
    ...
    const startToMatchPlayer = (id: string): MatchPlayer => { const player = startById.get(id); return { id, displayName: player.displayNameSnapshot, gender: player.genderSnapshot, skillWeight: skillWeight(player.skillLevelSnapshot as SkillLevel), skillLevel: player.skillLevelSnapshot, status: player.status, gamesPlayed: player.matchesPlayed, queueEnteredAt: player.queueEnteredAt, lastMatchEndedAt: player.lastMatchEndedAt, manualPriority: player.manualPriority, latePenaltyState: player.latePenaltyState, latePenaltyAppliedAt: player.latePenaltyAppliedAt }; };
    ...
    if (match.matchmakingMode === MatchmakingMode.BALANCED || match.matchmakingMode === MatchmakingMode.MIXED_DOUBLES || match.matchmakingMode === MatchmakingMode.GUIDED) {
      ...
      if (match.matchmakingMode === MatchmakingMode.MIXED_DOUBLES) {
        ...
        if (mixedError) throw conflict("MIXED_DOUBLES_COMPOSITION", mixedError);
      } else if (match.matchmakingMode !== MatchmakingMode.GUIDED) {
        const balanceError = validateBalancedLineup(teamA, teamB, Number((match.suggestionExplanation as any)?.strengthGap ?? 1));
...
api.post("/matches", requireAuth, requireMutationOrigin, route(async (request, response) => { const body = parse(z.object({ teamA: z.array(idSchema), teamB: z.array(idSchema), courtId: idSchema.optional(), suggestionToken: z.string().min(20).optional(), suggestionAdjusted: z.boolean().optional(), playerPreferenceConfirmed: z.boolean().optional() }), request.body); const suggestionPayload = body.suggestionToken ? await validateSuggestionRequest(request, body.suggestionToken, body.teamA, body.teamB, Boolean(body.suggestionAdjusted)) : undefined; responseData(response, matchView(await createMatch(request, { ...body, suggestionPayload })), 201); }));
...
api.post("/matches/start-suggestion", requireAuth, requireMutationOrigin, route(async (request, response) => { const body = parse(z.object({ teamA: z.array(idSchema).length(2), teamB: z.array(idSchema).length(2), courtId: idSchema, suggestionToken: z.string().min(20), playerPreferenceConfirmed: z.boolean().optional() }), request.body); const suggestionPayload = await validateSuggestionRequest(request, body.suggestionToken, body.teamA, body.teamB, false); responseData(response, matchView(await createMatch(request, { ...body, suggestionToken: body.suggestionToken, suggestionPayload })), 201); }));
...
api.patch("/matches/:id", requireAuth, requireMutationOrigin, route(async (request, response) => {
  ...
  const updatedId = await withTransactionRetry(async (tx) => {
    ...
    if (match.matchmakingMode === MatchmakingMode.GUIDED) validateGuidedMutation(teamAPlayers, teamBPlayers, synergyError);
    else if (synergyError) throw conflict("SYNERGY_TEAM_LINEUP", synergyError);
    else if (match.source === MatchSource.AUTOMATIC && isProhibitedGeneratedGenderMatch(teamAPlayers, teamBPlayers)) throw conflict("GENERATED_GENDER_RULE", "Generated matchups cannot place two female players against two male players.");
    ...
    const preservedGuided = match.matchmakingMode === MatchmakingMode.GUIDED;
    ...
    await tx.match.update({ where: { id: match.id, version: expected }, data: { ...courtData, source: MatchSource.MANUAL_ADJUSTED, matchmakingMode: preservedBalanced ? MatchmakingMode.BALANCED : preservedGuided ? MatchmakingMode.GUIDED : null, algorithmVersion: preservedBalanced || preservedGuided ? (match.algorithmVersion ?? MATCHMAKING_ALGORITHM) : null, suggestionKey: null, suggestionExplanation: previousExplanation || updatedAdvisory || preservedBalanced || preservedGuided ? { ...(previousExplanation ?? {}), ...((preservedBalanced || preservedGuided) ? { algorithmVersion: match.algorithmVersion ?? MATCHMAKING_ALGORITHM, adjusted: true } : {}), ...(preservedGuided ? guidedExplanation : {}), matchupAdvisory: updatedAdvisory } : null, version: { increment: 1 } } });
